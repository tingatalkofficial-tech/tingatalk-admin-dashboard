...
import { EarningsStat } from '../../types/analytics';

interface EarningsCardProps {
  stat: EarningsStat;
}

const EarningsCard: React.FC<EarningsCardProps> = ({ stat }) => {
    ...
    ">
      ...
      <div className="flex flex-col gap-[12px]">
        <span className="font-['Urbanist'] text-[22px] sm:text-[24px] font-bold leading-[26px] text-[#232d2c]">
          {stat.currency === 'INR' ? '₹' : ''}{stat.amount.toLocaleString()}
          {stat.currency === 'COINS' ? ' coins' : ''}
        </span>
